function trajectory = tspsolver(inPoints,cmat)
...
fprintf(fileID,'%s\n','NODE_COORD_SECTION :');
for ii = 1:dimension
    fprintf(fileID,'%s\n',[num2str(ii) ' ' num2str(inPoints(ii,1))...
        ' ' num2str(inPoints(ii,2))]);
end
fprintf(fileID,'%s\n','EDGE_WEIGHT_SECTION :');
[rows,colm] = size(cmat);
for ii = 1:rows
    fprintf(fileID,'%s',num2str(cmat(ii,:)));
    fprintf(fileID,'%s\n','');
end
fprintf(fileID,'%s\n','EOF');
fclose(fileID);
...
fclose(fileID);
trajectory = inPoints(trajectoryind,:);
